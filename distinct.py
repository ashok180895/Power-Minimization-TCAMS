...
with open('/home/exam/kk/project/project_iiitdm/acl_5k/acl1_5K.rl','r') as f1:
	lines1=f1.readlines();
	...
	#llp=0
	with open('/home/exam/kk/project/project_iiitdm/acl_5k/acl1_5K.trace','r') as f_tr:
...
#	print(len(cou_sip))
	...
	with open('dest_port.txt','r') as f_dport:
		...
		print(len(tr_dport))
	with open('protocol.txt','r') as f_protocol:
		tr_protocol=f_protocol.readlines()
		print(len(tr_protocol))
	for i in range(0,len(tr_lines)):
		cou_sip=[]
		cou_dip=[]
		cou_sport=[]
		cou_dport=[]
	...
	#print(ak4)
		cou_protocol=[]
		for xcv1 in range(0,len(tr_protocol)):
			if xcv1 in cou_dport:
				if(protocol_trace[i] == tr_protocol[xcv1]):
					cou_protocol.append(xcv1)
		ak5.append(len(cou_protocol))				
	for io in range(0,len(tr_lines)):
		ak1[io]=ak1[io]+ak2[io]+ak3[io]+ak4[io]+ak5[io]
	print("completed the formal travesing with searches=====",ak1)
	sum_1=sum(ak1)
	print(sum_1)
	for i in range(0,len(tr_lines)):
		cou_dip=[]
		...
		cou_protocol=[]
		for xcv2 in range(0,len(tr_protocol)):
			if xcv2 in cou_dport:
				if(protocol_trace[i] == tr_protocol[xcv2]):
					cou_protocol.append(xcv2)
		ak15.append(len(cou_protocol))	

		for xz in range(0,len(tr_sport)):
			if xz in cou_protocol:
				ss=tr_sport[xz].split(":")
				sk1=ss[0]
				#print(sk1)
				sk2=ss[1]
	...
	for a in range(0,len(tr_sip)):
		ak11[a]=ak11[a]+ak12[a]+ak13[a]+ak14[a]+ak15[a]
		ak1[a]=ak1[a]-ak11[a]
	print(ak1)
	sum_2=sum(ak1)
	print(sum_2)
